 ...
 * A schema is generated by an array of models :
 * 
 * ```php
 * $dbSchema = array(
 * 'user' => array(
 ...
 *	)
 * ));
 * $schema = new Schema($dbSchema);
 * $schema->generateTables();
 * ```
 * 
 * This code creates a user table with 3 columns (id, username, password) in database.
 * The schema class can then create models using : 
 * 
 * ```php
 * $userModel = $schema->getModel('user');
 * ```
 * 
 * See below for the schema available methods.
 * 
 * @subpackage Data models
 */
class Schema {
	...
	/**
	 * Gets the model from the schema data.
	...
	/**
	 * Truncates all the tables associated to the schema data.
	 * @return bool TRUE if the tables were truncated correctly, FALSE otherwise.
 	 * @subpackage Data models
	 */
	...
	/**
	 * Destroys all the tables associated to the schema data.
	 * @return bool TRUE if the tables were destroyed correctly, FALSE otherwise.
 	 * @subpackage Data models
	 */
	...
	/**
	 * Generates all the tables associated with the schema data.
	 * @return TRUE if all the tables have been correctly created, FALSE otherwise.
 	 * @subpackage Data models
	 */
...
/**
 * <h2>Models</h2>
 * Models are the way to handle data validation and read/write operations on the database
 * 
 * ```php
 * $userModel
 * 	->insert(array('username' => 'Georges', 'password' => sha1('GeorgesLucas123')))
 *	->insert(array('username' => 'Georges', 'password' => sha1('GeorgesWashington123')))
 * 	->commit();
 * $users = $userModel->get();
 * ```
 * 
 * They have ORM features like where, using, join, orderBy, groupBy, limit, offset methods.
 * @subpackage Data models
 * 
 */
class Model {
	...
	public function __construct($modelName) {
		...
		if( !$model ){
			throw new Exception("Cannot create model " . $modelName . " : unknow schema data.");
		}
		if( !isset($model['fields']) ){
			throw new Exception("Cannot create model " . $modelName . " : malformed field data.");
		}
	...
	/**
	 * Gets all the field from the model
	...
	/**
	 * Generates an HTML representation of a field
	...
	/**
	 * Generates multiple fields
	 * @param array $fields The fields to display
	 * @param callable $callback A callback to use to generate the HTML
	 * @return string The HTML content to display
 	 * @subpackage Data models
	 */
	public function generateFields($fieldsToDisplay, $callback=null){
	 ...
	 * 
	 * @param array $options The CRUD options :
	 * <ul>
	 ...
	 * <li>/model/read</li>
	 * <li>/model/edit/[0-9]+</li>
	 * <li>/model/delete</li>
	 * </ul>
	 * 
	 * They take care of insertion and deletion operations as well as the data validation.
	 * 
 	 * @subpackage Data models
	 */
	...
	/**
	 * Does a SELECT operation.
	 * @param string|array $select An array of column selectors, or just a string one.
	...
	/**
	 * Does a GROUP BY operation
	 * @param string|array $groupBy Column name(s) ```php $userModel->groupBy(array('username')); ```
	 * @return Model The model
 	 * @subpackage Data models
	 */
	...
	/**
	 * Does an ORDER BY operation
	 * @param string|array $orderBy ```php $userModel->orderBy(array('username ASC', 'created_at DESC')); ```
	 * @return Model The model
 	 * @subpackage Data models
	 */
	...
	/**
	 * Does an LIMIT operation.
	 * If the limit is set to 1, the get() operation will return an associative array of column/value pairs
	 * @param int $limit Limit of results to return ```php $userModel->limit(5); ```
	 * @return Model The model
 	 * @subpackage Data models
	 */
	...
	/**
	 * Does an OFFSET operation.
	 * @param int $offset Offsets the results by $offset ```php $userModel->offset(30); ```
	 * @return Model The model
 	 * @subpackage Data models
	 */
	...
	/**
	 * Uses a relation field to query on it
	 * @param string|array $modelPath 
	 * @param string|null $alias The alias to use when querying this relation
	 * @param bool $selectFields If set to TRUE, force the field selection
	 * @return type
 	 * @subpackage Data models
	 */
	...
	/**
	 * Joins model with an SQL table
	 * @param array $joins The join array.
	 * @return Model The model
 	 * @subpackage Data models
	 */
	...
	/**
	 * Does a WHERE operation on the schema.
	 * If there are many of this one, WHERE operations are concatened by an AND operation.
	 * @param array $where ```php $userModel->where('username LIKE ' . sql_quote('Georges%')) ```
	 * @return Model The model
 	 * @subpackage Data models
	 */
	...
	/**
	 * Does an HAVING operation
	 * @param array $where ```php $players = $userModel->select('SUM(score)')->having('score>0'); ``` 
	 * @return Model The model
 	 * @subpackage Data models
	 */
	...
	/**
	 * Gets the result of the model query.
	 * @return mixed Generally, returns an array of associative data. But, you can limit the results to 1 to get an associative array.
 	 * @subpackage Data models
	 */
	public function get(){
	 ...
	 * Call commit() to effectively insert it.
	...
	/**
	 * Replaces data with new value
	 * @param type $modelPath The relation to use. An empty string by default 
	 * @param array $data The data to replace with
	 * @return Model The model
 	 * @subpackage Data models
	 */
	public function replace($modelPath='', array $data = array()) {
	 ...
	 * @param string $deletion The relation to delete
	 * @return Model The model
 	 * @subpackage Data models
	 */
	...
	/**
	 * Commits all operations on database (insert,replace,delete) and resets the model
	 * @return Model The model
	 */
